...
% Status: FINAL FIX (Corrected Wavenumbers + CFL Safety)
% =========================================================================

function DNS_Master_Solver
    clearvars; clc; close all;

    % --- 1. COMPUTATIONAL DOMAIN SETUP ---
    N = 128;                % Grid Resolution
    L = 2*pi;               % Box Length
    nu = 0.005;             % Viscosity 
    t_final = 1000.0;         % Simulation time
    
    % --- 2. SPECTRAL OPERATORS (FIXED) ---
    % Standard MATLAB FFT Wavenumber ordering.
    % This fixes the "Masking Error" from the previous version.
    k_vec = [0:N/2-1 -N/2:-1]; 
    [Kx, Ky] = meshgrid(k_vec, k_vec);
    
    % Laplacian Operator in Fourier Space (-k^2)
    K2 = Kx.^2 + Ky.^2;
    K2(1,1) = 1e-6; % Prevent division by zero
    
    % De-aliasing Mask (The 2/3 Rule)
    k_max = N/2;
    % Now correctly filters high freq including Nyquist
    mask = (abs(Kx) < 2/3 * k_max) & (abs(Ky) < 2/3 * k_max); 

    % --- 3. INITIAL CONDITIONS (Taylor-Green Vortex) ---
    ...
    % --- 4. MAIN INTEGRATION LOOP (Runge-Kutta 4) ---
    t = 0;
    iter = 0;
    
    % Initial safe time step
    dt = 0.001; 
    
    while t < t_final
        
        % --- A. CFL SAFETY CHECK (New) ---
        % We calculate the max velocity to ensure dt is safe.
        % This prevents the "Numerical Instability" error.
        if mod(iter, 10) == 0
             ...
             % CFL = 0.5 (Standard stability limit)
             target_dt = 0.5 * (dx / max_vel);
             
             % Smoothly adjust dt (don't jump too fast)
        ...
        if mod(iter, 50) == 0
            ...
            % If this still triggers, the physics is broken, not the code.
            if any(isnan(w_real(:)))
...
function rhs = ComputeRHS(w_hat, Kx, Ky, K2, mask, nu)
    ...
    % 3. Nonlinear Convection (Real Space)
    u = real(ifft2(u_hat));
    v = real(ifft2(v_hat));
    ...
    % 4. Transform back and apply De-aliasing
